...
#### Data parallelism
Data parallelism, on the other hand, refers to processing multiple pieces (technically *batches*) of data through multiple *replicas of the same network* located on different hardwares/devices. Unlike model parallelism, each replica is an entire network and not a part of it. This strategy, as you might have guessed, can scale up well with increasing amount of data. But, as the entire network has to reside on a single device, it cannot help models with high memory footprint. The illustration above (taken from [here](https://xiandong79.github.io/Intro-Distributed-Deep-Learning)) should make it clear.
...
Okay ! Now it's time to learn the tools of the trade - PyTorch's `torch.distributed` API. If you remember the lessons from last tutorial, which you should, we made use of the `mpi4py` package which is a convenient wrapper on top of original `Intel MPI`'s *C* library. Thankfully, PyTorch offers a very similar interface to use underlying Intel MPI library and runtime. Needless to say, being a part of PyTorch, the `torch.distributed` API has a very elegent and easy-to-use design. We will now see the basic usage of `torch.distributed` and how to execute it.
...
2. Two parameters need to be retrieved: the `rank` and `world size` which exactly what `dist.get_rank()` and `dist.get_world_size()` respectively does. Remember, the world size and rank totally depends on the context of `mpiexec`. In this case, world size will be 2; `miriad2a` and `miriad2b` will be assigned rank 0 and 1 respectively.

3. `x` is a tensor that Rank 0 intend to send to Rank 1. It does so by `dist.send(x, dst=1)`. Quite intuitive, isn't it ?

4. `z` is something that Rank 1 creates before recieving the tensor. Yes ! you need an already created tensor of *same shape* as a holder for catching the incoming tensor. The values of `z` will eventually be replaced by the values in `x`.

5. Just like `dist.send(..)`, the recieving counterpart is `dist.recv(z, src=0)` which recieves the tensor into `z`.

## Communication collectives:

What we saw in the last section is an example of "*peer-to-peer*" communication where rank(s) send data to specific rank(s) in a given context. Although this is usefull and gives you granular control over communication, there exist other standard and frequently used *pattern* of communication called `collective`s. A full-fledged explanation of these `collective`s is beyond the scope of this tutorial, but I will describe one paticular collective (known as `all-reduce`) which is of interest to us in the context of distributed training of deep models.
...
when launched in a *world* of 3 *ranks*, it produces
...
3. `x` from every rank is summed up and the summation is placed inside the same `x` of every node. That's how all-reduce works.

## Moving on to Deep learning

Well ! Enough of bare bone distributed computing. Let's dive into what we are actually here for - Deep learning. I assume the reader is familiar with the "*Stochastic Gradient Descent (SGD)*" algorithm which is often used to train deep learning models. We will now see a variant of GD (called **Synchronous SGD**) that makes use of the `all-reduce` collective. To lay the foundation, let's look into normal SGD's mathematical formulation:
...
The core trick that Synchronous SGD uses is splitting the summation over more smaller subsets of the (mini)batch. If $$D$$ is split into $$R$$ number of subsets $$D_1, D_2, .. D_R$$ (preferrably with same number of samples) such that

\\[
	D = \bigcup_{r=1}^{R} D_r
\\]

Now splitting the summation in the SGD formula leads to
...
Have a look at those individual $$\displaystyle{\nabla_{\theta}\sum_{D_r} Loss(X, y)}$$ terms. They can now be computed independently and summed up to get the original gradient without any loss/approximation. This is where the **data parallelism** comes into picture. Here is the whole story:

1. Split the *entire dataset* into $$R$$ equal chunks. The letter $$R$$ is used to refer to "*Replica*".
2. Launch $$R$$ workers using `MPI` and bind each worker to one chunk of the dataset.
3. Let each worker compute the gradient (with batch size $$B$$) using it's own chunk of data, i.e., $$r^{th}$$ worker computes $$\displaystyle{\nabla_{\theta}\sum_{D_r} Loss(X, y)}$$
4. Sum up all the gradients of all the workers and make the resulting gradient available to all of them to proceed further.

The last point should look familiar. That's exactly the `all-reduce` algorithm. So, we have to execute all-reduce everytime all workers have computed one gradient (on a minibatch of size $$B$$) on their own share of the dataset. *A subtle point to note here* is that summing up the gradients (on batches of size $$B$$) from all $$R$$ workers leads to an effective batch size of
...
1. All $$R$$ workers create their own copy/*replica* of the model with random weights.

2. Individual replicas with random weights *may* lead to initial de-synchronization. It is preferreble to synchronize the initial weights among all the replicas. The `sync_initial_weights(..)` does exactly that. Let any one of the worker "send" its weights to its siblings and the siblings must grab them to initialize themselves.
~~~python
...
3. Fetch a mini-batch (of size $$B$$) from the respective share of a worker and compute forward and backward (gradient) pass. Important note to remember here as a part of the [setup]() is all workers should have it's own share of data visible (usually on it's own hard-disk OR on a shared filesystem).
...
5. After gradients have been synchronized, every replica can execute an SGD update on it's own weight independently. The `optimizer.step()` does that job.
...
We already made sure that $$\theta_{initial}$$ and $$\nabla_{\theta} \sum_{D} Loss(X, y)$$ are synchronized individually (Point 2 & 4 above). For obvious reason, a linear combination of them will also be in sync ($$\lambda$$ is a constant).