...
and then, the caller/client has to *request for generating one element* at a time like so
...
Another lesser known but fairly advance usage of `yield` keyword is to *transfer/delegate* generation process to another generator(s). This is how it works:
