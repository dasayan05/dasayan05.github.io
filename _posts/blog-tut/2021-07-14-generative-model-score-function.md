...
Moreover, Eq. 5 has a very intuitive interpretation and this is where [Pascal Vincent, 2011](https://www.iro.umontreal.ca/~vincentp/Publications/DenoisingScoreMatching_NeuralComp2011.pdf) uncovered the link between DSM and Denoising Autoencoders. A closer look at Eq. 5 would reveal that the $$\mathbf{s}_{\theta}(\mathbf{\tilde{x}})$$ has a learning target of $$- \frac{1}{\sigma^2} (\mathbf{\tilde{x}} - \mathbf{x})$$, which can be interpreted as an unit vector from corrupted sample towards the real sample. Succintly, the score function is trying to learn how to "de-noise" a corrupted sample - that's precisely what Denoising Autoencoders do.
