...
<center>
    <img width="50%" style="padding-top: 20px; padding-bottom: 20px;" src ="/public/posts_res/11/uERdv.png" />
</center>
...
1. **Ligature** is a special *glyph* which is formed by joining two *glyphs* of two letters (or [grapheme](https://en.wikipedia.org/wiki/Grapheme)s). Canonical examples of ligature are the *grapheme* pair "ff" and "fi" both of which happened to be present in the content of the example sentence. The typeset sentence use just one *glyph* for both "ff" and "fi" which is not the case in word processors.

2. **(Non-)optimal spacing** does affect the appeal of the text when read by a human eye. The word "AVAST" clearly has more amount of space in the later case which indeed is very awful. If you want more of it, look at character-pair "Fe" and "Ta" in the words "Feline" and "Table" respectively.

3. **Hyphenation** is defined as the process of breaking words between lines. A better hyphenation algorithm can produce much less breakage of words in a given paragraph. The reason word processors are not-so-good at it is their hyphenation algorithm works on a single line and not on the entire paragraph (which is the case with the tools in `TeX` family). The below example should be self-explanatory.
<center>
    <img width="50%" style="padding-top: 20px; padding-bottom: 20px;" src ="/public/posts_res/11/hyphenation.png" />
</center>

There are many more of these. It's difficult to discuss all of them here. If you are interested, read articles like [this](http://www.rtznet.nl/zink/latex.php?lang=en) or many others by expert typographers. In this article, I would rather focus on the tools available for doing typesetting digitally.

Before we begin, it's good to have an idea about how exactly these typeset documents are produced *digitally*. They are done by means of some specially crafted file formats:
...
**Digital typesetting** refers to the process of typesetting in digital medium and produce high quality printing material. So, a digital typesetting system must consume the *content* and *formatting* of the material I want to print and produce a DVI/PS/PDF which can then be used by the traditional printers.
...
Okay, enough of history and vague descriptions. Let me introduce you to the language of `TeX`. In case you want to follow along, please install any complete `TeX`-distribution (`MikTeX`, `TeXLive` etc.) and you'll get all the required tools ready for use. Here's a simple `TeX` program:
...
```
`\kern` is there to produce a given amount of horizontal space. A negative amount will cause the next character to overlap. `\lower`, as you can guess, is for *lowering* the following *box* from its horizontal baseline.
...
whereas `LaTeX`'s abstraction will allows you to have much more *focus on content* rather than formatting details. Here's how you would do it in `LaTeX`
...
You can see the very basic `LuaLaTeX` command that makes the bridge between `LaTeX` with `Lua` is at work here. The `\directlua` command enables users to write arbitrary lua code inside it. Here's how it works:

1. The engine stops interpreting the usual `LaTeX` commands (i.e., stops typesetting) once it has encountered a `\luacode` block
2. The code inside this block is then fed into a special Lua interpreter for execution
3. The special `tex.print(..)` function (same `print()` API) injects the characters into a special output stream.
4. The `\luacode` block is then replaced by the content of the output stream.
5. `LaTeX` engine starts its starts again from it was halted.

Hopefully the explanation is clear enough to understand why the output looks this this
...
1. `\begin{luacode*} .. \end{luacode*}` is an environment to put pure Lua definitions. In our example, there are two functions namely `intro_helper_tex(..)` and `intro_helper_latex(..)`. They differ in the very last word of the `tex.print(..)` function.
2. To understand the reason for *escaping* the backslash, go read the 4th point of the earlier explanation very carefully. The output stream generated by the `tex.print(..)` has to be *valid (La)TeX code* to be successfully parsed subsequently by the `LaTeX` engine. Escaping the backslash produces "\TeX" as a string in the output stream which is a valid `TeX` command.
...
4. Depending on the second argument, the `if .. elseif .. end` block choses one of the two lua functions earlier defined.
...
<center>
    <img width="40%" style="padding-top: 20px; padding-bottom: 20px;" src ="/public/posts_res/11/lualatex_out2.1.png" />
</center>

---

Phew ! That was a hell of a lengthy tutorial; but hopefully conveys the essence of typesetting and the `TeX` family of tools. All the members of the `TeX` family are themselves huge systems to learn about. With the introductory ideas given in this tutorial, it will be easier to read their documentations available online.