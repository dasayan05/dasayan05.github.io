...
<a href="https://github.com/dasayan05/rlx" target="_blank" class="fa fa-github fa-3x" style="float: right;"></a>
...
for _ in range(k_epochs):
    # 'evaluate' an episode against a policy and get a new 'Rollout' object
    rollout = agent(policy).evaluate(base_rollout)
...
- A little modified (simplified) `SlimeVolleyGym-v0` [environment by David Ha](https://github.com/hardmaru/slimevolleygym). An MLP agent trained with PPO learns to play volleyball by self-play experiences, provided at `examples/slime.py`.
